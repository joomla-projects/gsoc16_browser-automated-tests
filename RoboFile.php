...
class RoboFile extends \Robo\Tasks
{
	...
	public function runSelenium()
	{
		...
		{
			$this->_exec("libraries/vendor/bin/selenium-server-standalone >> selenium.log 2>&1 &");
		}
		else
		{
			$this->_exec("START java.exe -jar .\\libraries\\vendor\\joomla-projects\\selenium-server-standalone\\bin\\selenium-server-standalone.jar");
		}
	...
	public function runTests($opts = ['use-htaccess' => false, 'env' => 'desktop'])
	{
		...
		$this->_exec('php libraries/vendor/bin/codecept build');

		$pathToCodeception = 'libraries/vendor/bin/codecept';
		...
		$this->taskCodecept($pathToCodeception)
			...
			->arg('--fail-fast')
			->arg('--env ' . $opts['env'])
			->arg('tests/acceptance/administrator.feature')
			->run()
			->stopOnFail();
		/*
		$this->taskCodecept($pathToCodeception)
	...
	public function runTest($pathToTestFile = null, $suite = 'acceptance')
	{
		...
		$this->taskCodecept('libraries/vendor/bin/codecept')
			->test($pathToTestFile)
			->arg('--steps')
			->arg('--debug')
			->run()
