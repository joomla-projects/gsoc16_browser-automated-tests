Feature: Users Frontend
  ...
  Scenario: Create user from frontend (index.php?option=com_users)
    ...
    And there is no user with Username "user2" or Email "user2@gmail.com"
    When I press on the link "Create an account"
    And I create a user with fields Name "user2", Username "user2", Password "user2" and Email "user2@gmail.com"
  ...
  Scenario: User can not login, if the account has not been activated
    Given A not yet activated user with username "user2" exists
    And I am on a frontend page with a login module
    When I enter username "user2" and password "user2" into the login module
  ...
  Scenario: Check if block and activation are working
    Given I am on the User Manager page
    When I unblock the user "user2"
    And I activate the user "user2"
    And I login with user "user2" with password "user2" in frontend
    Then I should see the message "Hi user2,"

  Scenario: Profile changes done in the frontend are available in the backend
    Given I am logged in into the frontend as user "user2" with password "user2"
    When I press on the "Edit Profile" button
    And I change the name to "user3"
    And I press the "Submit" button
    And I should see "Profile successfully saved." message
    And I am on the User Manager page
    And I search the user with name "user3"
    Then I should see the name "user3"

  Scenario: Test last login date
    Given Needs to user "user2" logged in at least once
    When I login as a super admin from backend
    Then I should see last login date for "user3"
