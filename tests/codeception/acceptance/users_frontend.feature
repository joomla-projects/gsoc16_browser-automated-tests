Feature: Users Frontend
  ...
  Scenario: Create user from frontend (index.php?option=com_users)
    ...
    And there is no user with Username "User Two" or Email "UserTwo@gmail.com"
    When I press on the link "Create an account"
    And I create a user with fields Name "User Two", Username "User Two", Password "pass2" and Email "UserTwo@gmail.com"
  ...
  Scenario: check the created user in the backend
    ...
    And I search the user with user name "User Two"
    Then I should see the user "User Two"
  ...
  Scenario: User can not login, if the account has not been activated
    ...
    When I enter username "User Two" and password "pass2" into the login module
  ...
  Scenario: Check if block and activation are working
    ...
    And I login with user "User Two" with password "pass2" in frontend
    Then I should see the message "Hi User Two,"

  Scenario: Profile changes done in the frontend are available in the backend
    Given I am logged in into the frontend as user "User Two" with password "pass2"
